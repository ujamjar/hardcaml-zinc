...
let value ?(chan = stdout) st v =
  ...
  in
  if is_int v = 1L
  then fprintf chan "=long%Li" (shift_right v 1)
  else if in_program v
  then fprintf chan "=code@%Li" (codeofs v)
  else if in_stack sp v
  then fprintf chan "=stack_%i" ((st.mapping.stack_address - to_int v) / 8)
  else if is_block v = 1L
  then (
    let h = header v in
