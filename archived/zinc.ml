...
let memory_if ~e ~stack_o ~bc_o ~mem_o ~ext_i =
  ...
  let req =
    concat_lsb
      (List.map m ~f:(fun (x : Always.Variable.t Memory.O.t) -> x.memory_request.value))
  in
  let gnt =
...
let comp_int op a b =
  ...
  zero (dbits - 2) @: mux op [ a ==: b; a <>: b; a <+ b; a <=+ b; a >+ b; a >=+ b ] @: vdd
;;
...
module M = Mlvalues.Make (struct
  ...
  let zero = const 0
  let one = const 1
  let log_shift f a b = if Signal.is_const b then f a (to_int b) else log_shift f a b
...
let zinc i =
  ...
  let bc_i =
    { bc_i with
      memory_data_in =
        sres
        @@ mux2 bc_sel.value bc_i.memory_data_in.:[63, 32] bc_i.memory_data_in.:[31, 0]
    }
  in
  ...
  let temp =
    Array.init n_temps ~f:(fun i ->
        ...
        g)
  in
  let clear_temps = proc (Array.to_list @@ Array.map temp ~f:(fun t -> t <--. 0)) in
  ...
  let access_memif mem_i mem_o =
    ...
    let ready =
      ...
      fun f -> when_ ready [ mem_o.memory_request <--. 0; proc (f mem_i.memory_data_in) ]
    in
    read, write, ready
  in
  (* reading bytecode *)
  let read_bytecode, when_bytecode_ready =
    ...
    let read nstate =
      proc [ bc_sel <-- pcsel; (* XXX 64/32 *) read pcaddr nstate; pc <-- pc_next ]
    in
    read, ready
  in
  ...
  let closure_nfuncs_offs =
    mux2 decode'.closure (one dbits) (sll (closure_nfuncs.value -- "nfuncs") 1 -:. 1)
  in
  ...
  let bcomp_int = (comp_int decode'.bcomp_op bc_i.memory_data_in accu.value).:[1, 1] in
  let bucomp_int = (ucomp_int decode'.bucomp_op bc_i.memory_data_in accu.value).:[1, 1] in
  let comp_int = comp_int decode'.comp_op accu.value stack_i.memory_data_in in
  let ucomp_int = ucomp_int decode'.ucomp_op accu.value stack_i.memory_data_in in
  ...
  compile
    ...
    ; state.switch
        ...
        ; (* fetch instruction *)
          ( `fetch
          , [ read_bytecode `decode; (* clear temporaries *) count <--. 0; clear_temps ] )
        ; (* decode instruction *)
          ( `decode
                  ...
                  [ if_
                    ...
                    @@ elif
                         ...
                         [ if_
                             ...
                             [ read_stack (sp.value +: aofs decode.acc_op) `acc_set ]
                         ]
                    (* push/acc/const *)
                    @@ elif
                         (decode.pushacc
                         |: decode.pushconst
                         |: decode.pushgetglobal
                         |: decode.pushgetglobalfield
                         |: decode.pushatom
                         |: decode.pushenvacc)
                         [ push_stack_accu `pushacc ]
                    @@ elif
                         decode.envacc
                         [ read_mem (env.value +: aofs (decode.envacc_op +:. 1)) `envacc ]
                    @@ elif
                         decode.const
                         [ if_
                             (msb decode.const_op)
                             [ read_bytecode `constint ]
                             [ accu <-- val_int decode.const_op; state.set_next `fetch ]
                         ]
                    (* branch *)
                    ...
                    @@ elif
                         decode.isint
                         [ accu <-- ures accu.value.:[0, 0]; state.set_next `fetch ]
                    (* closure(rec) *)
                    ...
                    @@ elif decode.vectlength [ read_mem (hdrp accu.value) `vectlength ]
        ...
        ; ( `acc_set
          , [ when_stack_ready (fun data -> [ accu <-- data; state.set_next `fetch ]) ] )
        ; ( `pushacc
          , [ when_stack_ready (fun _ ->
                  [ if_
                      decode'.pushconst
                      [ if_
                          (msb decode'.pushconst_op)
                          [ read_bytecode `constint ]
                          [ accu <-- val_int decode'.pushconst_op; state.set_next `fetch ]
                      ]
                    ...
                    @@ elif
                         ...
                         [ if_
                             (msb decode'.push_op)
                             [ read_bytecode `acc_offset ]
                             [ read_stack (sp.value +: aofs decode'.push_op) `acc_set ]
                         ]
                  ])
            ] )
        ; ( `envacc
          , [ when_mem_ready (fun data ->
                  ...
                  ; when_
                      ...
                      [ state.set_next `invalid_instruction (* not implemented yet! *) ]
                  ])
            ] )
        ; ( `pop
          , [ when_bytecode_ready (fun offset ->
        ...
        ; (* alu *)
          ( `alu
          , [ when_stack_ready (fun _ -> [ accu <-- alu_int; state.set_next `fetch ]) ] )
        ; (* comparison *)
          ( `comp
          , [ when_stack_ready (fun _ -> [ accu <-- comp_int; state.set_next `fetch ]) ] )
        ; ( `ucomp
          , [ when_stack_ready (fun _ -> [ accu <-- ucomp_int; state.set_next `fetch ]) ]
          )
        ; (* branch with comparison *)
          ...
          , [ when_bytecode_ready (fun _ ->
                  [ if_
                      ...
                      [ pc <-- pc_next; (* skip branch address *) state.set_next `fetch ]
                  ])
            ] )
        ; ( `bucomp
          , [ when_bytecode_ready (fun _ ->
        ...
        ; ( `makeblock_read
          , [ when_mem_ready (fun _ ->
                  [ if_
                      ...
                      [ accu <-- makeblock_accu_base.value; state.set_next `fetch ]
        ...
        ; ( `closure_var_start
          , [ accu <-- alloc_pointer.value
            ; when_mem_ready (fun _ ->
                  [ count <--. 0
                  ; if_ (closure_nvars.value <>:. 0) [ state.set_next `closure_var_read ]
                    @@ elif
                         decode'.closure
        ...
        ; ( `closure_var_write
          , [ when_stack_ready (fun data ->
                  let addr =
                    alloc_pointer.value +: aofs (count.value +: closure_nfuncs_offs)
                  in
        ...
        ; ( `closure_accu_pc1
          , [ when_bytecode_ready (fun ofs ->
                  [ write_mem accu.value (pc.value +: bcofs (ofs -:. 1)) `closure_accu_pc2
                  ])
            ] )
        ; `closure_accu_pc2, [ when_mem_ready (fun _ -> [ state.set_next `fetch ]) ]
        ; (* globals *)
          ( `setglobal
          , [ when_bytecode_ready (fun ofs ->
                  [ write_mem (i.globals_start_address +: aofs ofs) accu.value `fetch
                  ; accu <-- val_unit
                  ])
            ] )
        ; ( `getglobal_data
        ...
        ; ( `getglobalfield
          , [ when_mem_ready (fun data -> [ accu <-- data; state.set_next `fetch ]) ] )
        ; (* XXX TODO: C_CALLN *)
        ...
        ; ( `c_call3
          , [ when_stack_ready (fun _ ->
                  [ sp <-- sp.value +: aofs decode'.c_call_op; state.set_next `fetch ])
            ] )
        ; ( `offsetint
          , [ when_bytecode_ready (fun data ->
        ...
        ; ( `offsetref1
          , [ when_mem_ready (fun data ->
                  [ write_mem accu.value (data +: sll temp.(0).value 1) `offsetref2 ])
            ] )
        ; ( `offsetref2
          , [ when_mem_ready (fun _ -> [ accu <-- val_unit; state.set_next `fetch ]) ] )
        ; ( `atom
          , let get_tag ofs =
              mux2
                (decode'.atom
        ...
        ; (* applyX XXX TODO APPLY *)
          ...
          , [ when_stack_ready (fun d ->
                  ...
                  ; if_
                      ...
                      [ count <--. 0
                      ; push_stack (val_int extra_args.value) `apply_push_stack_env
        ...
        ; ( `apply_push_stack_env
          , [ when_stack_ready (fun _ -> [ push_stack env.value `apply_push_stack_pc ]) ]
          )
        ; ( `apply_push_stack_pc
          , [ when_stack_ready (fun _ -> [ push_stack pc.value `apply_push_stack_args ]) ]
          )
        ; ( `apply_push_stack_args
          , [ when_stack_ready (fun _ ->
        ...
        ; ( `grab
          , [ when_bytecode_ready (fun reqd ->
                  [ if_
                      ...
                      [ extra_args <-- extra_args.value -: reqd; state.set_next `fetch ]
                      [ (* XXX TODO *) state.set_next `invalid_instruction ]
                  ])
            ] )
        ; ( `push_retaddr0
          , [ when_bytecode_ready (fun ofs ->
                  [ push_stack (pc.value +: aofs ofs) `push_retaddr1 (* XXX ofs-1??? *) ])
            ] )
        ; ( `push_retaddr1
          , [ when_stack_ready (fun _ -> [ push_stack env.value `push_retaddr2 ]) ] )
        ; ( `push_retaddr2
          , [ when_stack_ready (fun _ ->
                  [ push_stack (val_int extra_args.value) `push_retaddr3 ])
            ] )
        ; `push_retaddr3, [ when_stack_ready (fun _ -> [ state.set_next `fetch ]) ]
        ; ( `vectlength
        ...
        ; ( `getvectitem1
          , [ when_mem_ready (fun data -> [ accu <-- data; state.set_next `fetch ]) ] )
        ; ( `setvectitem0
          , [ when_stack_ready (fun d -> [ temp.(0) <-- d; pop_stack `setvectitem1 ]) ] )
        ; ( `setvectitem1
          , [ when_stack_ready (fun d ->
                  [ write_mem (accu.value +: aofs (int_val temp.(0).value)) d `fetch ])
            ] )
        ; ( `setvectitem2
          , [ when_mem_ready (fun _ -> [ accu <-- val_unit; state.set_next `fetch ]) ] )
        ; ( `getstringchar0
        ...
        ; ( `setstringchar0
          , [ when_stack_ready (fun d -> [ temp.(0) <-- d; pop_stack `setstringchar1 ]) ]
          )
        ; ( `setstringchar1
          , [ when_stack_ready (fun d ->
        ...
        ; ( `setstringchar2
          , [ when_mem_ready (fun d ->
                  [ write_mem
                      accu.value
                      (set_byte temp.(1).value.:[3, 1] d temp.(0).value.:[8, 1])
                      `setstringchar3
                  ])
            ] )
        ; `setstringchar3, [ when_mem_ready (fun _ -> [ state.set_next `fetch ]) ]
  ...
  ; instruction =
      mux2 (state.is `decode &: bc_i.memory_ready) instruction (consti ~width:8 255)
  ; error = error.value
  ; memory_o
  ; decode = decode'
